// Demo Mode Interceptor & Mock Dataset
const isDemoMode = new URLSearchParams(window.location.search).get('demo') === 'true' || window.location.pathname.includes('demo');

if (isDemoMode) {
    console.log("[Google Assistant Entity Console] Demo mode active - using mock backend.");
    const mockState = {
        yaml_filename: "gaGen_072026.yaml",
        version: "1.6.0",
        blocklist: [".*_battery_level$", ".*_ble_.*", "sensor.debug_.*"],
        ai_settings: {
            ai_source: "openai",
        ...
        entities: [
            {
                ...
                name: "Ceiling Light",
                display_name: "Living Room Ceiling Light",
                domain: "light",
                area: "Living Room",
                floor: "Main Floor",
                exposed: true,
            ...
            },
            {
                ...
                name: "Floor Lamp",
                display_name: "Living Room Floor Lamp",
                domain: "light",
                area: "Living Room",
                floor: "Main Floor",
                exposed: true,
                aliases: ["Standing Lamp", "Reading Light"],
                is_group_member: false,
                state: "off"
            },
            {
                entity_id: "light.kitchen_pendant_light_1",
                name: "Pendant Light 1",
                display_name: "Kitchen Island Pendant",
                domain: "light",
                area: "Kitchen",
                floor: "Main Floor",
                exposed: true,
                aliases: ["Island Light"],
            ...
            },
            {
                ...
                display_name: "Main Floor Thermostat",
                domain: "climate",
                area: "Hallway",
                floor: "Main Floor",
                exposed: true,
                aliases: ["Thermostat", "AC", "Heater"],
                is_group_member: false,
                state: "heat_cool"
            },
            {
            ...
            },
            {
                ...
                name: "Living Room TV",
                display_name: "Living Room OLED TV",
                domain: "media_player",
                area: "Living Room",
                floor: "Main Floor",
                exposed: true,
                aliases: ["TV", "Big Screen"],
                is_group_member: false,
                state: "off"
            },
            {
                ...
                display_name: "Downstairs Vacuum",
                domain: "vacuum",
                area: "Living Room",
                floor: "Main Floor",
                exposed: false,
                aliases: ["Roborock", "Dusty"],
                is_group_member: false,
                state: "docked"
            },
            {
                ...
                name: "Smart Plug",
                display_name: "Kitchen Coffee Maker",
                domain: "switch",
                area: "Kitchen",
                floor: "Main Floor",
                exposed: false,
                aliases: ["Coffee Machine"],
                is_group_member: false,
                state: "off"
            },
            {
                ...
                name: "Bedroom Aircon",
                display_name: "Master Bedroom AC",
                domain: "climate",
                area: "Master Bedroom",
                floor: "Second Floor",
                exposed: true,
            ...
            {
                entity_id: "light.master_bed_lamp",
                name: "Bedside Lamp",
                display_name: "Master Bedside Lamp",
                domain: "light",
                area: "Master Bedroom",
                floor: "Second Floor",
                exposed: false,
    ...
    window.fetch = async function(url, options = {}) {
        ...
        if (urlStr.includes('/api/google_assistant_entity_console/')) {
            const body = options.body ? (typeof options.body === 'string' ? JSON.parse(options.body) : options.body) : {};
            
            if (urlStr.endsWith('/entities')) {
                return new Response(JSON.stringify({
                    entities: mockState.entities,
                    yaml_filename: mockState.yaml_filename,
                    version: mockState.version
                }), { status: 200, headers: { 'Content-Type': 'application/json' } });
            }
            
            if (urlStr.endsWith('/blocklist')) {
                if (options.method === 'POST') {
                    if (body.blocklist) mockState.blocklist = body.blocklist;
                    return new Response(JSON.stringify({ success: true, blocklist: mockState.blocklist }), { status: 200, headers: { 'Content-Type': 'application/json' } });
                }
                return new Response(JSON.stringify({ blocklist: mockState.blocklist }), { status: 200, headers: { 'Content-Type': 'application/json' } });
            }
            
            if (urlStr.endsWith('/blocklist/add')) {
                if (body.pattern && !mockState.blocklist.includes(body.pattern)) {
                    mockState.blocklist.push(body.pattern);
                }
                return new Response(JSON.stringify({ success: true, blocklist: mockState.blocklist }), { status: 200, headers: { 'Content-Type': 'application/json' } });
            }

            if (urlStr.endsWith('/rebuild') || urlStr.endsWith('/restart') || urlStr.endsWith('/entities/update')) {
                return new Response(JSON.stringify({ success: true }), { status: 200, headers: { 'Content-Type': 'application/json' } });
            }

            if (urlStr.endsWith('/ai/settings')) {
                if (options.method === 'POST') {
                    mockState.ai_settings = { ...mockState.ai_settings, ...(body.settings || {}) };
                    return new Response(JSON.stringify({ success: true, settings: mockState.ai_settings }), { status: 200, headers: { 'Content-Type': 'application/json' } });
                }
                return new Response(JSON.stringify({ settings: mockState.ai_settings }), { status: 200, headers: { 'Content-Type': 'application/json' } });
            }

            if (urlStr.endsWith('/ai/models')) {
                return new Response(JSON.stringify({
                    models: [
            ...
            if (urlStr.endsWith('/ai/ha_agents')) {
                return new Response(JSON.stringify({
                    agents: [
                        { id: 'conversation.home_assistant', name: 'Home Assistant Assist' },
                        { id: 'conversation.ollama_agent', name: 'Ollama Local Agent' }
                    ]
                }), { status: 200, headers: { 'Content-Type': 'application/json' } });
            }

            if (urlStr.endsWith('/ai/generate_nicknames')) {
                const results = {};
                (body.entities || []).forEach(e => {
                    results[e.entity_id] = ["Smart Light", "Accent Lamp", "Room Light"];
                });
                return new Response(JSON.stringify({ results }), { status: 200, headers: { 'Content-Type': 'application/json' } });
            }

            if (urlStr.endsWith('/ai/generate_single_nickname')) {
                return new Response(JSON.stringify({
                    aliases: ["Main Light", "Overhead Illumination", "Room Lamp"]
                }), { status: 200, headers: { 'Content-Type': 'application/json' } });
            }

            if (urlStr.endsWith('/ai/suggest_exposure')) {
                const exposed_ids = mockState.entities
                    .filter(e => e.domain === 'light' || e.domain === 'climate')
                    .map(e => e.entity_id);
                return new Response(JSON.stringify({ exposed_ids }), { status: 200, headers: { 'Content-Type': 'application/json' } });
            }
        }
...
// Auto-trigger view and modal states for screenshot capture in demo mode
if (isDemoMode) {
    const demoParams = new URLSearchParams(window.location.search);
    if (demoParams.get('view') === 'domain') {
        window.addEventListener('DOMContentLoaded', () => {
            setTimeout(() => {
                const select = document.getElementById('groupingType');
                if (select) {
                    select.value = 'domain';
    ...
    if (demoParams.get('modal') === 'blocklist') {
        window.addEventListener('DOMContentLoaded', () => {
            setTimeout(() => {
